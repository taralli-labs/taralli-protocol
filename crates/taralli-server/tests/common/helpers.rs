...
use serde_json::Value;
use taralli_server::{
    config::Config, error::Result, routes::subscribe::SubscribeQuery, state::BaseState,
...
pub async fn subscribe(app: Router, system_ids: &[ProvingSystemId]) -> BodyDataStream {
    let query = system_ids
        .iter()
        .map(|id| format!("system_ids={}", id.as_str()))
        .collect::<Vec<_>>()
        .join("&");

    let request = Request::builder()
        .method("GET")
        .uri(format!("/subscribe?{}", query))
        .header("Accept", "text/event-stream")
        .body(Body::empty())
        .unwrap();
    ...
    P: Provider<T, Ethereum> + Clone,
{
    ...
    // Each receiver is turned into a stream of SSE events
    let streams = receivers.into_iter().map(|rx| {
        BroadcastStream::new(rx).map(|result| {
            result
    ...
    // Return SSE
    Ok(Sse::new(merged).keep_alive(KeepAlive::new().interval(std::time::Duration::from_secs(15))))
}
...
pub async fn setup_app(size: Option<usize>) -> (Router, ValueState<Http<Client>, RootProvider<Http<Client>>>) {
    let config = Config {
        server_port: 8080,
        rpc_url: "http://localhost:8545".to_owned(),
        log_level: "DEBUG".to_owned(),
        ...
        market_address: Address::default(),
        common_validation_config: CommonValidationConfig::default(),
        request_validation_config: RequestSpecificConfig {
            maximum_allowed_stake: 0,
        },
        offer_validation_config: OfferSpecificConfig {
            maximum_allowed_reward: Some(U256::ZERO),
            minimum_allowed_stake: Some(U256::ZERO),
        },
    };

    let rpc_provider = ProviderBuilder::new().on_http(config.rpc_url().unwrap());
    let subscription_manager: SubscriptionManager<Value> =
        SubscriptionManager::new(size.unwrap_or(3));
    ...
    (Router::new()
        .route("/submit/request", axum::routing::post(submit_handler_json))
        .route("/subscribe", axum::routing::get(subscribe_handler_json))
        .with_state(value_state.clone())
        .layer(TraceLayer::new_for_http()), value_state)
}
*/